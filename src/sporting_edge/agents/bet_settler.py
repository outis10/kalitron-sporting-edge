...
async def run_bet_settler() -> dict:
    ...
    async with AsyncSessionLocal() as db:
        ...
        async with FootballAPIClient() as client:
            for bet in bets:
                result = await _get_fixture_result(client, bet.match_id)
                if result is None:
                    # Match not finished yet or API error — try again next cycle
                    summary["skipped"] += 1
                    continue

                actual_outcome, home_goals, away_goals = result
                settlement_price = _determine_settlement(
